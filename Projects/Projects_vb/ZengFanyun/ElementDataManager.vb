...
#Region "  ---  Fields"
    ...
    Private ActiveMonitorData As MonitorData
...
#Region "   ---   本地子方法"
    ...
    Private Sub SaveTableToElement(sender As Object, e As EventArgs) Handles btnSaveChange.Click
        ...
        With Me.eZDataGridView1
            ...
            For r = 0 To RowsCount - 2
                ...
                Next
            Next
            Dim moniData As New MonitorData(arrDate, arrValue)
            strData = BinarySerializer.Encode64(moniData)
        End With

        Dim aa As Parameter = ActiveElement.Parameter(GuidMonitor)

        Dim para As Parameter = ActiveElement.Parameter(GuidMonitor) ' ActiveElement.Parameter_MonitorData
    ...
    Private Function FillTableWithElementData(ele As Element, Table As DataGridView) As Boolean
        ...
        If strData.Length > 0 Then
            Dim Dt As MonitorData
            Try
                Dt = DirectCast(BinarySerializer.Decode64(strData), MonitorData)
    ...
    Private Function DrawData(ByVal Data As MonitorData) As Chart_MonitorData
        Dim Chart1 As New Chart_MonitorData(GlobalSettings.FamilyName.沉降测点)
        With Chart1
