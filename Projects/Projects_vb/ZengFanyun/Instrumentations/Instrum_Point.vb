...
#Region "   ---   构造函数"
        ...
        ''' <remarks></remarks>
        Friend Sub New(MonitorPoint As FamilyInstance, ByVal Type As InstrumentationType)
...
#End Region
        ...
        Private Function GetData() As MonitorData_Point
            Dim strData As String = Monitor.Parameter(Constants.SP_Monitor_Guid).AsString
            Dim MData As MonitorData_Point = Nothing
            If strData.Length > 0 Then
                Try
                    MData = DirectCast(StringSerializer.Decode64(strData), MonitorData_Point)
                    If (MData.arrDate Is Nothing) OrElse (MData.arrValue Is Nothing) Then
                        Throw New Exception
                    End If
                    Return MData
                Catch ex As Exception
                    TaskDialog.Show("Error", Me.Monitor.Name & " (" & Me.Monitor.Id.IntegerValue.ToString & ")" & " 中的监测数据不能正确地提取。", TaskDialogCommonButtons.Ok, TaskDialogResult.Ok)
                    Return Nothing
                End Try
            End If
            Return MData
        ...
        Public Function SaveData() As Boolean
            If Me.MonitorData IsNot Nothing Then
                ' 将数据序列化为字符串
                Dim strData As String = StringSerializer.Encode64(Me.MonitorData)
                Dim para As Parameter = Monitor.Parameter(Constants.SP_Monitor_Guid) ' ActiveElement.Parameter_MonitorData
